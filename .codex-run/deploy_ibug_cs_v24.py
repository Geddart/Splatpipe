"""Redeploy IBUG --cluster-sh to FRESH IBUG_cs_v24.

v24 = v23 + Phase-1 viewer changes (template-only; fork rcf2 UNCHANGED,
scene .rad/.radc UNCHANGED):
  * #48 item #6 — per-scene `clip_xy`. The old global hard-coded
    `sparkOpts.clipXY = 3.0` is gone; default is now Spark's own 1.4
    (project.py DEFAULT_SCENE_CONFIG.spark_render.clip_xy + viewer
    _DEFAULTS), read per-scene from viewer-config, `?clipXY=N` override.
    IBUG ships at the 1.4 default (NO clip_xy injected here) — must be
    Playwright-verified for no blanking under aggressive orbit/zoom.
    Speicher etc. carry clip_xy:3.0 in their own config at their redeploy.
  * #50 — applySplatBudget() now snaps the on-screen budget <select> to
    the actual lodSplatCount (single source of truth; UI/screenshot
    self-consistency for any programmatic budget change).

Everything else identical to v23 (Safari hint, probe/rotate benches, #81
FOCUS_MAX_AHEAD=0, 16 M pool, adaptive probe). Fork-version-agnostic:
asserts the CURRENT SPARK_FORK_URL (must still be the clean rcf2). Fresh
folder => new .radc paths => no edge-cache carry-over.
"""
...
assert FORK_TAG == "rcf2", (
    ...
    f"v24 is a template-only change; no fork rebuild is involved."
)
...
# NOTE: deliberately NO clip_xy here — IBUG ships at the new 1.4 default and
# is verified for no blanking post-deploy. If it blanks, add
# cfg["spark_render"]={"clip_xy": <v>} and redeploy.
...
assert "spark_render" not in back or "clip_xy" not in back.get("spark_render", {}), \
    "IBUG must NOT carry clip_xy — it ships at the 1.4 default for verification"
print("injected probe_views: %d poses; start_view preserved: %s; clip_xy: default 1.4" % (
...
if result and result.summary:
    ...
    print("CLIPXY A/B:    ", f"{cdn}/{FRESH}/index.html?clipXY=3", flush=True)
    print("HINT FORCED ON:", f"{cdn}/{FRESH}/index.html?safariHint=1", flush=True)
