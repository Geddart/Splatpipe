"""Redeploy IBUG --cluster-sh to FRESH IBUG_cs_v23.

Carries (vs v19, the current user-confirmed-good build):
  * EVERYTHING in v19 unchanged: clean rcf2 fork + #81 FOCUS_MAX_AHEAD=0
    near-field fix + adaptive probe bench + rotate bench + bench dropdown.
  * NEW: the Safari-on-Mac streaming hint (#78). A dismissible in-viewer
    notice shown ONLY when the WebGL renderer string is ANGLE+Metal
    (Chromium/Firefox on a Mac) — the exact discriminator that proved the
    unfixable ANGLE→Metal sync-staged-upload jitter. Safari, Windows/Linux
    and iOS are all correctly excluded; ?safariHint=1|0 forces on/off.

This is the agreed Phase-4 remediation of #78 (root cause exhausted: 3 fork
upload rewrites + WebGPU + ANGLE's own off-switch all failed). Fork is
UNCHANGED (rcf2) — TS/HTML-only template change, no fork rebuild.

Script stays fork-version-agnostic: it asserts against the *current*
SPARK_FORK_URL imported from the template. Fresh folder => new .radc paths
=> no edge-cache carry-over.
"""
...
# rcfN token is the source of truth in the template, not hard-coded here.
...
assert FORK_TAG == "rcf2", (
    f"expected the clean rcf2 fork (v19 known-good baseline), got {FORK_TAG}. "
    f"#78 is shipped as B (accept); no fork rebuild is involved."
)
...
# --- viewer-config.json: preserve live config, inject probe_views ---
...
cfg["probe_views"] = good  # pos/quat/fov consumed by _probeViews()
...
checks = {
    ...
    "safari_hint": ('<div id="safari-hint"' in html
                    ...
                    and "location.search).get('safariHint')" in html),
...
if result and result.summary:
    ...
    print("PROBE BENCH:   ", f"{cdn}/{FRESH}/index.html?bench=probe", flush=True)
